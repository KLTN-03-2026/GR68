...
app.post('/create-payment-url', (req, res) => {
    const { amount, orderId, invoiceId, orderInfo } = req.body;
    
    // Use invoiceId if provided (for rent), otherwise use orderId (for store), fallback to timestamp
    const txnRef = invoiceId || orderId || `${Date.now()}`;
